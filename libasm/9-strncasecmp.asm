...
.loop:
    ...
    jz .equal       ; If n == 0, equality up to n

    mov al, byte [rdi]  ; Load byte s1
    mov bl, byte [rsi]  ; Load byte s2
...
.null_s1:
    ...
    mov al, 0
    mov bl, byte [rsi]
    sub al, bl      ; Will be negative...I think?
    movsx eax, al
    jmp .done

.null_s2:
    ; s2 is null, s1 has more characters, player 1 is always better
    mov al, byte [rdi]
    mov bl, 0
    sub al, bl      ; Will be positive...Right?
    movsx eax, al
    jmp .done
