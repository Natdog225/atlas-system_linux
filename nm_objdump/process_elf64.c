...
int process_elf64(int fd, const char *filename, const char *prog_name,
				  bool swap_endian)
{
	Elf64_Ehdr ehdr;
	...
	/* Swap section headers if necessary */
	if (swap_endian)
	{
		for (i = 0; i < ehdr.e_shnum; i++)
			swap_endian_elf64_shdr(&shdr_table[i]);
	}

	/* 3. Read Section Header String Table */
	if (ehdr.e_shstrndx != SHN_UNDEF && ehdr.e_shstrndx < ehdr.e_shnum)
	{
		Elf64_Shdr *shstrtab_shdr = &shdr_table[ehdr.e_shstrndx];
	...
	if (symtab_shdr->sh_entsize == 0) /* Avoid division by zero */
	{
	...
	/* Swap symbols if you're feelin it */
	if (swap_endian)
	{
		for (i = 0; i < num_symbols; i++)
			swap_endian_elf64_sym(&symtab[i]);
	}

	/* 7. Process and Print Symbols */
	for (i = 0; i < num_symbols; i++)
	{
		Elf64_Sym current_sym = symtab[i];
		char sym_type;
		const char *sym_name = "";
		...
		if (current_sym.st_name != 0)
		{
			...
			if (sym_type != '?' && strlen(sym_name) > 0 && sym_type != 'a')
			{
				...
				{
					printf("%016lx %c %s\n",
						   (unsigned long)current_sym.st_value,
						   sym_type, sym_name);
				}
			}
		}
