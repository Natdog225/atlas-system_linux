...
void swap_endian_elf64_ehdr(Elf64_Ehdr *ehdr)
{
	...
	ehdr->e_entry     = __builtin_bswap64(ehdr->e_entry);
	ehdr->e_phoff     = __builtin_bswap64(ehdr->e_phoff);
	ehdr->e_shoff     = __builtin_bswap64(ehdr->e_shoff);
...
void swap_endian_elf64_shdr(Elf64_Shdr *shdr)
{
	...
	shdr->sh_flags     = __builtin_bswap64(shdr->sh_flags);
	shdr->sh_addr      = __builtin_bswap64(shdr->sh_addr);
	shdr->sh_offset    = __builtin_bswap64(shdr->sh_offset);
	shdr->sh_size      = __builtin_bswap64(shdr->sh_size);
	shdr->sh_link      = __builtin_bswap32(shdr->sh_link);
	shdr->sh_info      = __builtin_bswap32(shdr->sh_info);
	shdr->sh_addralign = __builtin_bswap64(shdr->sh_addralign);
	shdr->sh_entsize   = __builtin_bswap64(shdr->sh_entsize);
}
void swap_endian_elf64_sym(Elf64_Sym *sym)
{
	...
	sym->st_value = __builtin_bswap64(sym->st_value);
	sym->st_size  = __builtin_bswap64(sym->st_size);
}
